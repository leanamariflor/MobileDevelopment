...
class MonthFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        homeViewModel.monthAnimeList.observe(viewLifecycleOwner) { animeList ->
        ...
        apiService.getHomeData().enqueue(object : Callback<HomeResponse> {
            override fun onResponse(call: Call<HomeResponse>, response: Response<HomeResponse>) {
                if (response.isSuccessful) {
                    val animeList = response.body()?.data?.top10Animes?.month ?: emptyList()
                    homeViewModel.monthAnimeList.postValue(animeList)
                }
            }
